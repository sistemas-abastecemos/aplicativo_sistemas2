...
<sub>Versión 1.2 · Documento maestro </sub>
...
| **Alcance del documento**     | Sistema completo: Frontend React, Backend PHP (cPanel — legacy + API v1 pública/privada), Framework de repositorios (servidor LAN), Base de datos MySQL / PostgreSQL, Infraestructura Cloudflare + Cloudflared |
| **Versión del aplicativo documentado** | v1.2 (última actualización: 24 de julio de 2026)                                                                                                                    |
| **Fecha de la documentación inicial** | 14 de julio de 2026                                                                                                                                                 |
| **Fecha de última actualización** | 28 de julio de 2026 — basada en `CHANGELOGS.md` del cliente                                                                                                            |
| **Fuente del análisis inicial** | `frontend.zip`, `backend.zip`, `repo.zip`, `mysqlphpmyadmin.sql`, `logo.png`                                                                                              |
| **Confidencialidad**          | Uso interno — no distribuir fuera de la organización                                                                                                                         |

---

## 0 · Cambios desde la documentación inicial (v1.x + v1.2)

Este bloque resume las **actualizaciones aplicadas a la documentación** para reflejar cambios recientes del aplicativo. Detalle por documento en el propio archivo modificado.

### 0.1 · Versión 1.x — 17 de julio de 2026

**Bloques mayores:**

- **API v1 (pública + privada)** — dos superficies API nuevas con núcleo compartido (`bootstrap.php`, `Response.php`, `Controller.php`), envelope estándar `success/meta/data|error`, `X-Request-ID`. Introducidas para consumo externo formal (pública) y como patrón moderno interno (privada). Ver [02](./02-arquitectura-general.md), [03 §5.3](./03-arquitectura-backend.md), [09 §3](./09-api-endpoints.md).
- **`ProveedoresRepo`** en framework LAN — 3 acciones nuevas: `general/listar_proveedores`, `general/buscar_proveedores_biable`, `general/buscar_proveedor_id`. Alimentan la superficie pública.
- **`api_keys` con hash SHA-256** — llaves ya no en texto plano. `hash_equals` en tiempo constante, scopes, IP allowlist. Fallback dual para migración sin downtime. Ver [10 §6](./10-autenticacion.md).
- **`dashboard_utilidades`** — nueva tabla + módulo AdminPanel para configurar bloque de "Utilidades" del dashboard, con visibilidad filtrada por área. Ver [14 §5](./14-base-de-datos.md), [04 §12.2](./04-arquitectura-frontend.md).
- **Modo oscuro con DarkReader** — hook `useDarkMode.ts` (primer TypeScript del proyecto) + toggle en topbar. Ver [04 §12.1](./04-arquitectura-frontend.md).
- **Rate limiting** aplicado en `api/v1/public` (30/min por API key) — cierre parcial de DT-005.
- **Headers de seguridad** en `.htaccess` de superficies v1 (X-Content-Type-Options, X-Frame-Options, Referrer-Policy, sin X-Powered-By) — cierre parcial de DT-014, DT-015.
- **`update_informe.php` defensivo** + **`create_informe.php` auto-orden** — ver [modulos/informes.md](./modulos/informes.md).
- **Bug fix `usePrefijosDian`** — excepción silenciosa que causaba falsa notificación de error.

### 0.2 · Versión 1.2 — 24 de julio de 2026

- **Silent SSO con Microsoft** — detección automática de sesión corporativa al cargar `/login` usando `prompt=none` OAuth 2.0 + iframe oculto + circuit breaker con `sessionStorage`. Ver [10 §4.5](./10-autenticacion.md).
- **Refactor del módulo Auth** — descomposición de `Login.jsx` monolítico en 7 componentes + 2 hooks + 1 utilitario siguiendo estrictamente SRP. Es el nuevo ejemplo canónico del patrón thin orchestrator del proyecto. Ver [04 §11.1](./04-arquitectura-frontend.md).
- **Fix de bucle infinito de logout** — banderas `user_logged_out` y `ms_silent_login_attempted` en `sessionStorage` + `?logout=true` en URL para evitar re-autenticación automática tras logout manual. Ver [24 §19](./24-codigo-explicado.md).

### 0.3 · Documentos afectados por estos cambios

**Pase 1** (Bloque API v1): 13 documentos — 02, 03, 05, 09, 10, 11, 12, 14, 15, 22, 24, 25, 26.
**Pase 2** (Silent SSO): 4 documentos — 04, 06, 10, 24.
**Pase 3** (Informes + PrefijosDIAN + Dark Mode + Dashboard): 6 documentos — 04, 13, 22, modulos/informes, modulos/contabilidad, modulos/admin-panel.
**Pase 4** (transversales): este README, 01, 27, 28.

Total: **26 documentos actualizados** con contenido nuevo o modificado.
